...
def main(gh_access_token, start_date_str, end_date_str, selected_repository, selected_members):
    ...
    for member in members:
        ...
        if username in selected_members or selected_members == ANY:
...
if __name__ == "__main__":
    ...
    selected_repository = 'fertiscan-backend,fertiscan-frontend,fertiscan-pipeline,nachet-backend,nachet-frontend,nachet-model,howard,github-workflows,devops,finesse-backend,finesse-frontend,finesse-data'
    repos = selected_repository.split(',')
    if not repos:
        repos = ANY

    selected_members = os.getenv("SELECTED_MEMBERS")
    selected_members = 'ThomasCardin'
